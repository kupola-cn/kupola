...
> **注意**：本文档部分内容为历史分析。文中引用的 `composition-api.js`、`KupolaHttp`、`KupolaRouter`、`KupolaTestUtils`、`kupola-devtools.js` 等模块已在 Phase 1 架构精简中删除。相关“已修复”标记表示该问题在删除前已确认，模块本身已不再存在。
...
后端开发者无需学习 JS 框架，只需在 HTML 模板上加属性即可使用完整功能。

### 2.4 精密生命周期状态机

`kupola-lifecycle.js` 的 `KupolaLifecycle` 类提供了比 Vue/React 更显式的状态机：
...
**修复方向**：让 `KupolaComponent` 在 `mount()` 前设置 `currentSetupContext`（使用 `SetupContext`），执行 `setup()` 函数，在 `mount()` 后触发 `onMounted` 回调。具体见 [composition-api.js](../js/composition-api.js) 的 `SetupContext` 和 [component.js](../js/component.js) 的 `KupolaComponent`。
...
**现状**：`kupola-core.js`（旧 nimbus-core.js）第 643-653 行，`removeEventListener` 传入新的箭头函数，与 `hashchange`/`popstate` 注册时的不是同一引用。
...
**现状**：`kupola-core.js`（旧 nimbus-core.js）第 793-801 行，引用 `activeRequests` Map，但 `_request` 方法从未向其写入，取消永远返回 false。
...
**现状**：`KupolaComponent`（旧 nimbus-core.js 第 109-114 行）用简单的 `beforeMount/afterMount/render` 钩子，与 `kupola-lifecycle.js` 那套带 priority/depends/trace 的精密状态机毫无关联。
...
**现状**：`KupolaStatePersist`（旧 nimbus-core.js 第 980-1057 行）和 `KupolaDataBind.persist`（data-bind.js 第 530-553 行）几乎相同，且都在 DOMContentLoaded 自动加载 `kupola:` 前缀存储。

**修复方向**：统一为单一实现，删除其中一份。

**已修复**：删除了旧 `nimbus-core.js` 中的 `KupolaStatePersist` 类，将其功能完全收归到 `data-bind.js` 的 `KupolaDataBind.persist/unpersist/loadPersisted`。同时增强了三点健壮性：(1) storage 容量检测，quota 超限时自动清理过期数据并降级到 sessionStorage；(2) schema 版本号支持，数据结构变更时自动跳过旧版本数据；(3) 加密选项，敏感数据持久化前可加密存储（依赖 CryptoJS）。
...
**现状**：旧 `nimbus-core.js` 第 467、506 行硬编码 `setTimeout(resolve, 300)`，`setAnimation` 方法设置了 `animationClass` 但实际未使用。
...
**现状**：旧 `nimbus-core.js` 中基于 fetch 的 `_request()` 方法，`response.ok` 从未被检查，4xx/5xx 错误响应会被当作成功响应处理。
...
**修复方案**：拆分为独立文件：`component.js`、`registry.js`、`router.js`、`http.js`，`kupola-core.js` 作为聚合器保持向后兼容。
...
**Kupola 的正确方向**：Kupola 不需要 SSR（它的首屏已经是服务端渲染的 HTML）。但需要**预渲染增强**——服务端输出 HTML 时，直接在模板中添加正确的 `data-component`/`data-bind`/`data-validate` 属性，减少客户端扫描后的“闪烁”。
...
| 编号 | 任务 | 影响范围 | 预估工作量 | 状态 |
|------|------|---------|-----------|------|
| 1.1 | 接通 Composition API 与组件系统 | composition-api.js + kupola-core.js | 中 | ⚠️ 部分 |
| 1.2 | 修复 `computed()` 依赖收集 | composition-api.js | 小 | ✅ 完成 |
| 1.3 | 修复 `reactive()` 嵌套访问 bug | composition-api.js | 小 | ✅ 完成 |
| 1.4 | 修复 Router `destroy()` 监听器移除 | kupola-core.js | 小 | ✅ 完成 |
| 1.5 | 修复 HTTP `cancelRequest` | kupola-core.js | 小 | ✅ 完成 |
...
| 2.2 | 组件类接入 KupolaLifecycle 状态机 | kupola-core.js + kupola-lifecycle.js | 大 | ⚠️ 部分 |
| 2.3 | 合并表单验证双轨为单一引擎 | form.js + validation.js | 中 | ✅ 完成 |
| 2.4 | 删除重复的状态持久化实现 | kupola-core.js / data-bind.js | 小 | ✅ 完成 |
...
| 3.3 | 路由动画改用 animationend 事件 | kupola-core.js | 小 | ✅ 完成 |

### Phase 4：工程化补全 ✅ 已完成

| 编号 | 任务 | 影响范围 | 预估工作量 | 状态 |
|------|------|---------|-----------|------|
| 4.1 | HTTP 模块改用 fetch，axios 作为可选 | kupola-core.js | 中 | ✅ 完成 |
...
| 4.3 | 修正类型定义偏差 | types/kupola.d.ts | 中 | ✅ 完成 |
...
| 编号 | 任务 | 影响范围 | 预估工作量 | 状态 |
|------|------|---------|-----------|------|
| 5.1 | 修复 KupolaHttp._request() 不检查 response.ok | kupola-core.js | 小 | ✅ 完成 |
| 5.2 | 修复 KupolaComponent.setState/setProps 未 await lifecycle.update | kupola-core.js | 小 | ✅ 完成 |
...
| 7.3 | 拆分核心文件为独立模块 | kupola-core.js → router.js, http.js, component.js, registry.js | 大 | ✅ 完成 |
| 7.4 | 统一组件初始化模式为声明式优先 | 所有组件 | 大 | ✅ 完成 |
| 7.5 | 统一 KupolaComponent 生命周期为 lifecycle.on() 模式 | kupola-core.js | 中 | ✅ 完成 |
| 7.6 | 添加错误边界机制 | kupola-lifecycle.js, kupola-core.js | 中 | ✅ 完成 |
...
所有前端能力都基于 `data-*` 属性声明，后端开发者只需在模板中加属性，无需写 JS 即可使用完整功能。
...
**架构重构（已完成）**：
- ✅ `kupola-core.js`（旧 nimbus-core.js）拆分 → 拆分为 component.js、registry.js、router.js、http.js，kupola-core.js 作为聚合器保持向后兼容
...
| `kupola-lifecycle.js` | `onError` 属性与方法同名 | ✅ 已修复 |
...
| **实例存储** | 通过 `element._kupolaXXX` 存储实例引用 |
...
| **实例管理** | 通过 `element._kupolaXXX` 存储实例，便于访问和控制 |
...
| TypeScript 类型定义 | ✅ | `types/kupola.d.ts` 完整 |
