...
从 `2.0.3` 开始，AI Adapter 会先解析自然语言，再进入中间件管道。因此中间件中的 `ctx.command` 已经是结构化命令，可以在执行任何 query/action/flow 之前做权限判断、审计或拦截。

从 `2.0.3` 开始，推荐使用 `adapter.capability.register()` 集中声明资源、权限、参数规则、返回字段和 handler，再用 `adapter.capability.middleware()` 统一拦截。
...
`adapter.capability.middleware()` 会根据已经注册的能力，在 handler 执行前做权限和参数校验。
...
| 选项 | 说明 |
|------|------|
| `strict` | 为 `true` 时，未注册到 capability 的 command 会被拒绝 |

::: tip
即使不挂载 `capability.middleware()`，通过 `adapter.capability.register()` 注册出来的 handler 仍会在执行时做权限、参数和结果过滤。中间件主要用于更早地给出拦截提示。
:::
