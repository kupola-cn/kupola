...
describe('lazy', () => {
  ...
  test('lazyComponent prefers an explicit named export over default', async () => {
    const namedExport = () => ({ element: 'named' });
    const defaultExport = () => ({ element: 'default' });
    const LazyComp = lazyComponent(
      () => Promise.resolve({ default: defaultExport, Named: namedExport }),
      'Named',
    );
  ...
  test('lazyComponent validates loader and export name', () => {
    expect(() => lazyComponent(null)).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), '')).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), ' ')).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), null)).toThrow(TypeError);
  });

  test('lazyComponent retries after a loader failure', async () => {
    ...
    const loader = jest.fn(() => {
      ...
      return attempts === 1
        ...
        : Promise.resolve({ default: () => ({ element: 'recovered' }) });
    });
    const LazyComp = lazyComponent(loader);

    await expect(LazyComp()).rejects.toBe(failure);
    const result = await LazyComp();
  ...
  test('lazyComponent shares a failed in-flight request and retries the next request', async () => {
    const failure = new Error('temporary failure');
    let rejectLoader;
    let attempts = 0;
    const loader = jest.fn(() => {
      attempts++;
      if (attempts === 1) {
        return new Promise((_, reject) => { rejectLoader = reject; });
      }
      return Promise.resolve({ default: () => ({ element: 'recovered' }) });
    });
    const LazyComp = lazyComponent(loader);
    ...
    await expect(LazyComp()).resolves.toEqual({ element: 'recovered' });
    expect(loader).toHaveBeenCalledTimes(2);
  });

  test('lazyComponent retries after a synchronous loader failure', async () => {
    let attempts = 0;
    const LazyComp = lazyComponent(() => {
      attempts++;
      if (attempts === 1) {throw new Error('sync failure');}
      return { default: () => ({ element: 'recovered' }) };
    });

    await expect(LazyComp()).rejects.toThrow('sync failure');
    await expect(LazyComp()).resolves.toEqual({ element: 'recovered' });
  });

  test('lazyComponent preserves falsy loader rejections and remains retryable', async () => {
    let attempts = 0;
    const LazyComp = lazyComponent(() => {
      attempts++;
      return attempts === 1
    ...
    await expect(LazyComp()).rejects.toBeNull();
    await expect(LazyComp()).resolves.toEqual({ element: 'recovered' });
  });
  ...
  test('preloadComponent throws error for non-lazy components', async () => {
    ...
    await expect(preloadComponent(null)).rejects.toThrow(
      '[Kupola preload] Argument must be a lazy component from lazyComponent()',
    );
  });
  ...
  test('lazyComponent times out a pending load and retries', async () => {
    let resolveFirst;
    let attempts = 0;
    const loader = jest.fn(() => {
      attempts++;
    ...
    const LazyComp = lazyComponent(loader, 'default', { timeout: 10 });

    const first = LazyComp();
    await expect(first).rejects.toMatchObject({ name: 'TimeoutError' });
    expect(LazyComp._isResolved()).toBe(false);

    resolveFirst({ default: () => ({ element: 'late' }) });
    await expect(LazyComp()).resolves.toEqual({ element: 'recovered' });
    expect(loader).toHaveBeenCalledTimes(2);
  });

  test('lazyComponent shares cancellation with concurrent callers', async () => {
    let resolveFirst;
    let attempts = 0;
    ...
    expect(firstError.message).toBe('route changed');
    resolveFirst({ default: () => ({ element: 'late' }) });
    expect(LazyComp._isResolved()).toBe(false);
    await expect(LazyComp()).resolves.toEqual({ element: 'recovered' });
    expect(loader).toHaveBeenCalledTimes(2);
  ...
  test('lazyComponent propagates an external abort signal and can retry', async () => {
    const controller = new AbortController();
    let resolveLoader;
    const loader = jest.fn(() => new Promise(resolve => { resolveLoader = resolve; }));
    const LazyComp = lazyComponent(loader, 'default', { signal: controller.signal });
    const pending = LazyComp();

    await Promise.resolve();
    controller.abort('unmounted');
    await expect(pending).rejects.toMatchObject({ name: 'AbortError', message: 'unmounted' });
    resolveLoader({ default: () => ({ element: 'late' }) });

    controller.abort();
    await expect(LazyComp()).rejects.toMatchObject({ name: 'AbortError' });
    expect(loader).toHaveBeenCalledTimes(1);
  });

  test('lazyComponent rejects invalid loading options', () => {
    expect(() => lazyComponent(() => Promise.resolve({}), 'default', null)).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), 'default', { timeout: -1 })).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), 'default', { timeout: Infinity })).toThrow(TypeError);
    expect(() => lazyComponent(() => Promise.resolve({}), 'default', { signal: {} })).toThrow(TypeError);
  });
});
