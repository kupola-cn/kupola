...
function loadDevTools() {
  ...
  if (!themeScript || !themeScript.src) return;
  
  const scriptPath = themeScript.src.substring(0, themeScript.src.lastIndexOf('/') + 1);
  
  const devToolsScript = document.createElement('script');
  devToolsScript.src = scriptPath + 'kupola-devtools.js?v=15';
...
if (typeof module !== 'undefined' && module.exports) {
  ...
  if (!window.__kupolaThemeInitialized) {
    window.__kupolaThemeInitialized = true;
    ...
    if (document.readyState === 'loading') {
      document.addEventListener('readystatechange', function readyStateHandler() {
        if (document.readyState !== 'loading') {
